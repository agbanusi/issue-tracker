...
suite('Functional Tests', function() {
    ...
    suite('PUT /api/issues/{project} => text', function() {
      ...
      test('No body', function(done) {
        ...
        .end(function(err, res){
          assert.equal(res.status, 200);
          assert.equal(res.text,'Wrong Id or something went wrong')
      ...
      test('One field to update', function(done) {
        chai.request(server)
        .put('/api/issues/test')
        .send({_id:"5ed2dd86c1f6832ebb06a781",issue_title:'Head'})
        .end(function(err, res){
          assert.equal(res.status, 200);
      ...
      test('Multiple fields to update', function(done) {
        ...
        .put('/api/issues/test')
        .send({_id:"5ed2dd86c1f6832ebb06a781",issue_title:'Head',
              issue_text:'Update',created_by:"admin"})
        .end(function(err, res){
          assert.equal(res.status, 200);
          assert.equal(res.text,"Successfully Updated")
    ...
    suite('GET /api/issues/{project} => Array of objects with issue data', function() {
      ...
      test('Multiple filters (test for multiple fields you know will be in the db for a return)', function(done) {
        chai.request(server)
        .get('/api/issues/test')
        .query({open:true,created_by:'John'})
        .end(function(err, res){
          assert.equal(res.status, 200);
          assert.isArray(res.body);
          assert.property(res.body[0], 'issue_title');
    ...
    suite('DELETE /api/issues/{project} => text', function() {
      ...
      test('Valid _id', function(done) {
        chai.request(server)
        .delete('/api/issues/test')
        .send({_id:"5ed2dd86c1f6832ebb06a781"})
        .end(function(err, res){
          assert.equal(res.status, 200);
